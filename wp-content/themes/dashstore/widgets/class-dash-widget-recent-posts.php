<?php /* Dash Recent Posts/Products */

if ( ! defined( 'ABSPATH' ) ) exit;

class Dash_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
	 		'dash_recent_posts',
			esc_html__('Dash Recent Posts', 'dashstore'),
			array('description' => esc_html__( "Dash Store special widget. Displaying number of recent posts on your site.", 'dashstore' ), )
		);
	}
	...
	?>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id("sort-by")); ?>"><?php esc_html_e('Sort by:', 'dashstore'); ?></label>
        	<select class="widefat" id="<?php echo esc_attr($this->get_field_id("sort-by")); ?>" name="<?php echo esc_attr($this->get_field_name("sort-by")); ?>">
          		<option value="date" <?php selected( $instance["sort-by"], "date" ); ?>><?php esc_html_e('Date', 'dashstore'); ?></option>
          		<option value="title" <?php selected( $instance["sort-by"], "title" ); ?>><?php esc_html_e('Title', 'dashstore'); ?></option>
          		<option value="comment_count" <?php selected( $instance["sort-by"], "comment_count" ); ?>><?php esc_html_e('Number of comments', 'dashstore'); ?></option>
				<option value="author" <?php selected( $instance["sort-by"], "author" ); ?>><?php esc_html_e('Author', 'dashstore'); ?></option>
				<option value="menu_order" <?php selected( $instance["sort-by"], "menu_order" ); ?>><?php esc_html_e('Menu Order (if specified only for pages)', 'dashstore'); ?></option>
           		<option value="rand" <?php selected( $instance["sort-by"], "rand" ); ?>><?php esc_html_e('Random', 'dashstore'); ?></option>
        	</select>
        </p>
        <p>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id("cats")); ?>"><?php esc_html_e( 'Show list of post Categories?', 'dashstore' ); ?></label>
        </p>
        <p>
	...
	public function widget($args, $instance) {
		...
		echo '<ul class="widget-posts-list recent-posts">';
				...
				<div class="content-wrapper">
					...
					<div class="entry-meta">
						...
						<?php if ($show_comments) :?>
							<div class="comments-qty"><i class="fa fa-comments-o"></i><?php comments_popup_link( esc_html__('No comments', 'dashstore'), esc_html__('1 comment', 'dashstore'), esc_html__('% comments', 'dashstore'), 'comments-link', esc_html__('Comments are off', 'dashstore') ); ?></div>
...
add_action( 'widgets_init', create_function( '', 'register_widget( "Dash_Recent_Posts" );' ) );
