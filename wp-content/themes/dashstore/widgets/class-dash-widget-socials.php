...
class Dash_Socials extends WP_Widget {

	function __construct() {
		parent::__construct(
			'dash_socials',
			esc_html__('Dash Social Icons', 'dashstore'),
			array(
				'description' => esc_html__('Dash Store special widget. Displays a list of social media website icons and a link to your profile.', 'dashstore'),
			),
			array(
				'width' => 600,
			)
	...
	function register_admin_styles() {
		wp_enqueue_style( 'dash-social-icons-widget-admin', get_template_directory_uri() . '/css/widget_socials_admin.css', true);
	}
	...
	?>
		<div class="social_icons_widget">
			...
			<p class="icon_options"><label for="<?php echo esc_attr($this->get_field_id('icon_size')); ?>"><?php esc_html_e('Icon Size:', 'dashstore'); ?></label>
			...
			<p class="label_options"><input type="checkbox" id="<?php echo esc_attr($this->get_field_id('show_title')); ?>" name="<?php echo esc_attr($this->get_field_name('show_title')); ?>" value="hide"<?php echo $checked; ?> /> <label for="<?php echo esc_attr($this->get_field_id('show_title')); ?>"><?php esc_html_e('Hide Title', 'dashstore'); ?></label></p>

			<?php if(esc_attr($instance['layout_type'] == 'inline')) { $checked = ' checked="checked"'; } else { $checked = ''; } ?>
			<p class="label_options"><input type="checkbox" id="<?php echo esc_attr($this->get_field_id('layout_type')); ?>" name="<?php echo esc_attr($this->get_field_name('layout_type')); ?>" value="inline"<?php echo $checked; ?> /> <label for="<?php echo esc_attr($this->get_field_id('layout_type')); ?>"><?php esc_html_e('Inline Mode', 'dashstore'); ?></label></p>
			...
			<?php if ($social_networks) {
					...
					<li>
						...
						<label for="<?php echo esc_attr($this->get_field_name('social_networks')); ?>[<?php echo esc_attr($key); ?>][label]"><?php esc_html_e('Label:', 'dashstore'); ?></label>
						...
						<label for="<?php echo esc_attr($this->get_field_name('social_networks')); ?>[<?php echo esc_attr($key); ?>][url]"><?php esc_html_e('URL:', 'dashstore'); ?></label>
						...
						<input class="widefat" type="hidden" id="<?php echo esc_attr($this->get_field_name('social_networks')); ?>[<?php echo esc_attr($key); ?>][title]" name="<?php echo esc_attr($this->get_field_name('social_networks')); ?>[<?php echo esc_attr($key); ?>][title]" value="<?php echo esc_attr( $value['title'] ); ?>" />
	...
	public function widget( $args, $instance ) {
		...
		foreach($social_networks as $network) :
			if ( $network['url'] && $network['url']!='' ) {
				...
				echo '<a href="'.esc_url($network['url']).'" target="_blank" title="'.esc_html__('Connect us on ', 'dashstore').esc_attr($network['title']).'"><i class="fa fa-'.esc_attr($network['icon']).esc_attr($icon_class).'"></i><span>'.esc_attr($network['label']).'</span></a>';
				echo '</li>';
			}
		endforeach;
		echo '</ul>';
...
add_action( 'widgets_init', create_function( '', 'register_widget( "Dash_Socials" );' ) );
